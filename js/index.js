...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("./json/libraries.json")
    ...
    .then((libraries) => {
      ...
      function filterLibraries() {
        const selectedQuadrants = Array.from(
          document.querySelectorAll("#quadrantFilter input:checked")
        ...
        const filteredLibraries = libraries.filter((library) =>
          selectedQuadrants.includes(library.quadrant)
        );
      ...
      window.goToSearchResults = function () {
  ...
  advancedSearchButton.addEventListener("click", function (event) {
    ...
    advancedMenu.style.display =
      advancedMenu.style.display === "none" ? "block" : "none";
  });
  ...
  async function performBookSearch({
    ...
    try {
      ...
      const matchedBooks = books.filter((book) => {
        ...
        const matchesGenres =
          selectedGenresLower.length > 0
            ? selectedGenresLower.some((genre) =>
                book.genres.some((g) => g.toLowerCase() === genre)
              )
            : true;
  ...
  if (releaseFrom || releaseTo) {
    searchParamsText += `| Published Between: ${releaseFrom || "Any"} - ${
      releaseTo || "Any"
    }`;
  }
  ...
  if ((results.length === 0) & searchParamsContainer) {
    ...
    results.forEach((book) => {
      ...
      bookDiv.addEventListener("click", () => {
        ...
        window.location.href = `book_details.html?book=${encodeURIComponent(
          book.name
        )}`;
      });
